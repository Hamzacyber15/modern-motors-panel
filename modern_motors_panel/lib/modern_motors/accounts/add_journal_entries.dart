...
//     return {
//       'id': 'entry_${DateTime.now().millisecondsSinceEpoch}_${accountId}',
//       'accountID': accountId,
//       'accountName': accountName,
//       'description': description.isNotEmpty ? description : 'Journal Entry',
...
//       'journalNbr': referenceNumber ?? _generateJournalNumber(),
//       'entries': entries.map((entry) => entry.toMap()).toList(),
//     };
//   }

//   String _generateJournalNumber() {
//     return 'J${DateTime.now().millisecondsSinceEpoch}';
//   }
// }
...
//   factory JournalEntryResponse.fromMap(Map<String, dynamic> data) {
...
//   static Future<JournalEntryResponse> saveJournalEntry(
//     JournalEntryRequest request,
//   ) async {
//     try {
//       final validationError = _validateJournalRequest(request);
//       if (validationError != null) {
//         return JournalEntryResponse(success: false, message: validationError);
//       }

//       final HttpsCallable callable = functions.httpsCallable('saveJournal');
//       final result = await callable.call(request.toMap());

//       final responseData = result.data as Map<String, dynamic>;
//       return JournalEntryResponse.fromMap(responseData);
...
//   static String? _validateJournalRequest(JournalEntryRequest request) {
...
//       if (entry.accountId == null || entry.accountId!.isEmpty) {
//         return 'All entries must have an account selected';
//       }

//       if (entry.debit <= 0 && entry.credit <= 0) {
//         return 'Each entry must have either debit or credit amount';
//       }

//       if (entry.debit > 0 && entry.credit > 0) {
//         return 'Each entry must have either debit OR credit, not both';
//       }
//     }

//     final totalDebit = request.entries.fold(
//       0.0,
//       (sum, entry) => sum + entry.debit,
//     );
//     final totalCredit = request.entries.fold(
//       0.0,
//       (sum, entry) => sum + entry.credit,
//     );

//     if ((totalDebit - totalCredit).abs() > 0.01) {
//       return 'Journal entry must be balanced. Debit: OMR ${totalDebit.toStringAsFixed(2)}, Credit: OMR ${totalCredit.toStringAsFixed(2)}';
//     }

//     final hasDebit = request.entries.any((entry) => entry.debit > 0);
//     final hasCredit = request.entries.any((entry) => entry.credit > 0);

//     if (!hasDebit || !hasCredit) {
//       return 'Journal entry must have at least one debit and one credit line';
//     }
...
// class AddJournalEntryWidget extends StatefulWidget {
//   const AddJournalEntryWidget({super.key});
...
//       entries: _entries.where((entry) => entry.isValid).toList(),
//     );
...
//     try {
//       final response = await JournalService.saveJournalEntry(request);
...
//         }
//       } else {
//         _showErrorSnackBar(response.message ?? 'Failed to save journal entry');
//       }
//     } catch (e) {
//       _showErrorSnackBar('Error saving journal entry: ${e.toString()}');
//     } finally {
//       if (mounted) {
//         setState(() {
//           _isSaving = false;
...
//         SizedBox(width: 8),
//         ElevatedButton.icon(
//           onPressed: (_isBalanced && !_isSaving) ? _saveEntry : null,
//           icon: _isSaving
//               ? SizedBox(
//                   width: 16,
//                   height: 16,
...
//                   )
//                 : Icon(Icons.save_outlined, size: 18),
//             label: Text(_isSaving ? 'Saving...' : 'Save Journal Entry'),
//             style: ElevatedButton.styleFrom(
//               backgroundColor: Color(0xFF3B82F6),
//               foregroundColor: Colors.white,
//               elevation: 0,
...
class JournalService {
  ...
  static Future<Map<String, dynamic>> saveJournalEntry({
    ...
    try {
      final validationError = _validateJournalRequest(
        branchId: branchId,
        entries: entries,
      );
      if (validationError != null) {
      ...
      final data = {
        ...
        'journalNbr':
            referenceNumber ?? 'J${DateTime.now().millisecondsSinceEpoch}',
        'entries': entries.map((entry) => entry.toMap()).toList(),
      };
      ...
      print('Sending to Cloud Function: $data');
      ...
      return {
        ...
        'journalId': responseData['journalId'],
      };
    } on FirebaseException catch (e) {
  ...
  static String? _validateJournalRequest({
    required String branchId,
    required List<JournalEntryLine> entries,
  }) {
    if (branchId.isEmpty) {
    ...
    final hasDebit = entries.any((entry) => entry.debit > 0);
    final hasCredit = entries.any((entry) => entry.credit > 0);
...
class _AddJournalEntryWidgetState extends State<AddJournalEntryWidget> {
  ...
  Future<void> _saveEntry() async {
    ...
    try {
      final response = await JournalService.saveJournalEntry(
        branchId: _selectedBranchId!,
        date: _selectedDate,
        entries: _entries.where((entry) => entry.isValid).toList(),
        referenceNumber: _referenceController.text.isNotEmpty
      ...
      if (response['success'] == true) {
        ...
        _showErrorSnackBar(
          response['message'] ?? 'Failed to save journal entry',
        );
      }
    } catch (e) {
      _showErrorSnackBar('Error saving journal entry: ${e.toString()}');
