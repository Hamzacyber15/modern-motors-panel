...
class _AddInventoryPageState extends State<AddEditProduct> {
  ...
  // Add include in inventory checkbox state
  bool _includeInInventory = true;
    ...
    Future.wait([
      ...
      setState(() {
        ...
        if (widget.isEdit && widget.productModel != null) {
          ...
          // Initialize includeInInventory from existing data
          _includeInInventory = product.includeInInventory ?? true;
  ...
  void submitProduct() async {
    ...
    loading.value = true;
    try {
      // If not included in inventory, set values to 0
      final threshold = _includeInInventory
          ? (double.tryParse(thresholdController.text.trim()) ?? 0)
          : 0;

      final m = _includeInInventory
          ? (double.tryParse(minimumController.text.toString()) ?? 0)
          : 0;

      final sp = _includeInInventory
          ? (double.tryParse(sellingPrice.text.toString()) ?? 0)
          : 0;

      final sku = _includeInInventory ? skuController.text : "";
      ...
      if (widget.isEdit && widget.productModel?.id != null) {
        ...
        final addProduct = ProductModel(
          ...
          subCategoryId: selectedSubCategoryId,
          description: descriptionController.text,
          minimumPrice: m.toDouble(),
          createAt: Timestamp.now(),
          createdBy: user.uid,
          sku: sku,
          sellingPrice: sp.toDouble(),
          includeInInventory: _includeInInventory, // Save the checkbox value
        );
        await DataUploadService.updateProduct(
              widget.productModel!.id!,
              addProduct,
  ...
  // Validator that only validates if included in inventory
  String? _conditionalValidator(
    String? value,
    String? Function(String?) validator,
  ) {
    if (!_includeInInventory) {
      return null; // Skip validation if not included in inventory
    }
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        child: Form(
                          ...
                          child: Column(
                            children: [
                              // Include in Inventory Checkbox
                              Container(
                                height: 44,
                                child: Row(
                                  children: [
                                    Checkbox(
                                      value: _includeInInventory,
                                      onChanged: (value) {
                                        setState(() {
                                          _includeInInventory = value ?? true;
                                        });
                                      },
                              ...
                              14.h,
                              // Conditional fields based on includeInInventory
                              if (_includeInInventory) ...[
                                Row(
                                  children: [
                                    Expanded(
                                    ...
                                    Expanded(
                                      child: CustomMmTextField(
                                        ...
                                        autovalidateMode:
                                            AutovalidateMode.onUserInteraction,
                                        validator: (value) => _conditionalValidator(
                                          value,
                                          (val) =>
                                              ValidationUtils.sellingPriceWithMinValidation(
                                                val,
