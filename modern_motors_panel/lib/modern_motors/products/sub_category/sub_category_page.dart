...
import 'package:modern_motors_panel/modern_motors/products/sub_category/product_subcategory_cardlist_view.dart';
...
class _ProductPageState extends State<SubCategoryPage> {
  ...
  List<ProductSubCategoryModel> allSubCategories = [];
  List<ProductCategoryModel> allCategories = [];
  List<ProductSubCategoryModel> displayedSubCategories = [];
  List<ProductCategoryModel> displayedCategories = [];
  ProductSubCategoryModel? categoryBeingEdited;
  ...
  List<List<dynamic>> getSubCategoryRowsForExcel(
    List<ProductSubCategoryModel> data,
  ) {
    return data.map((sub) {
      final categoryName = allCategories
          .firstWhere(
  ...
  void showAlert({
    bool isEdit = false,
    ProductSubCategoryModel? categoryModel,
    VoidCallback? onBack,
  }) {
    if (!mounted) return;
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
  ...
  void getSubCategory(ProductSubCategoryModel model) {
  ...
  Widget build(BuildContext context) {
    return Consumer<MmResourceProvider>(
      builder: (context, resource, child) {
        allSubCategories = resource.productSubCategoryList;
        return Column(
          children: [
            PageHeaderWidget(
              title: "Sub Category".tr(),
              buttonText: "Add Sub Category".tr(),
              subTitle: "Add your Sub categories".tr(),
              requiredPermission: 'Create Sub Category',
              selectedItems: selectedCategoryIds.toList(),
              buttonWidth: 0.34,
              onCreate: () {
                Future.microtask(() {
              ...
              onDelete: _deleteSelectedCategories,
              onPdfImport: () async {
                final rowsToExport = getSubCategoryRowsForExcel(
                  allSubCategories,
                );
                await PdfExporter.exportToPdf(
                  headers: subCategoryHeaders,
            ...
            allSubCategories.isEmpty
                ? EmptyWidget(text: "Sub category has not been added yet.".tr())
                : Expanded(
                    child: ProductSubCategoryCardListView(
                      productSubCategoriesList: allSubCategories,
                      selectedIds: selectedCategoryIds,
