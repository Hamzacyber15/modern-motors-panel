...
class SalesFilterDialog extends StatefulWidget {
  ...
  const SalesFilterDialog({
    ...
    required this.customers, // Add customers parameter
...
class _SalesFilterDialogState extends State<SalesFilterDialog> {
  ...
  void initState() {
    ...
    // Set initially selected customer if customerId exists
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                        ...
                        child: DropdownButtonFormField<String>(
                          ...
                          items: [
                            ...
                            ...widget.subCategories
                                .where(
                                  (subCat) => subCat.id == _filter.categoryId,
                                )
                                .map((subCategory) {
                                  return DropdownMenuItem(
                    ...
                    if (_filter.categoryId != null) const SizedBox(height: 16),
...
class _SalesListViewState extends State<SalesListView> {
  ...
  void _applyCombinedFilters() {
    ...
    if (_currentFilter.categoryId != null) {
      ...
      // This is a placeholder - adjust according to your SaleModel
      filtered = filtered.where((sale) {
        // Check if any item in the sale belongs to the selected category
        return sale.items.any(
          (item) => item.productId == _currentFilter.categoryId,
        );
      }).toList();
    }
    ...
    if (_currentFilter.subCategoryId != null) {
      filtered = filtered.where((sale) {
        // Check if any item in the sale belongs to the selected subcategory
        return sale.items.any(
          (item) => item.productId == _currentFilter.subCategoryId,
        );
      }).toList();
    }
