...
class NewPurchaseModel {
  ...
  factory NewPurchaseModel.fromFirestore(
    ...
    final batchAllocationsRaw = data['batchAllocations'];
    if (batchAllocationsRaw is List<dynamic>) {
      batchAllocations = batchAllocationsRaw.map((x) {
        if (x is Map<String, dynamic>) {
          return BatchAllocation.fromMap(x);
        } else {
          return BatchAllocation(
            batchId: '',
    ...
    // Handle batches used
    List<BatchUsed>? batchesUsed;
    final batchesUsedRaw = data['batchesUsed'];
    if (batchesUsedRaw is List<dynamic>) {
      batchesUsed = batchesUsedRaw.map((x) {
        if (x is Map<String, dynamic>) {
          return BatchUsed.fromMap(x);
        } else {
          return BatchUsed(
            batchId: '',
            productId: '',
    ...
    final productsUpdatedRaw = data['productsUpdated'];
    if (productsUpdatedRaw is List<dynamic>) {
      productsUpdated = productsUpdatedRaw.map((x) {
        if (x is Map<String, dynamic>) {
          return ProductUpdated.fromMap(x);
        } else {
          return ProductUpdated(
            productId: '',
            quantityDeducted: 0,
    ...
    final attachedDocumentsRaw = data['attachedDocuments'];
    if (attachedDocumentsRaw is List<dynamic>) {
      attachedDocuments = attachedDocumentsRaw
          .map((x) => x.toString())
          .toList();
    }
    ...
    return NewPurchaseModel(
      ...
      handlingCost: (data['handlingCost'] ?? 0).toDouble(),
      ...
      expectedDeliveryDate: (data['expectedDeliveryDate'] as Timestamp?)
          ?.toDate(),
      deliveryStatus: data['deliveryStatus']?.toString() ?? 'pending',
  ...
  NewPurchaseModel copyWith({String? status, DateTime? updatedAt}) {
    return NewPurchaseModel(
      id: id,
      branchId: branchId,
      supplierId: supplierId,
